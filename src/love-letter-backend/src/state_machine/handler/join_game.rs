...
use backend_framework::streaming::{MessageErrType, StreamSender};
use backend_framework::wire_api::proto_frj_ngn::proto_pre_game_message::{ProtoJoinGameAck, ProtoPlayerJoinMsg};
use backend_framework::wire_api::proto_frj_ngn::{ProtoGameType, ProtoPreGameMessage};
...
impl StateMachineEventHandler {
    ...
    pub fn join_game(
        ...
        match from_state {
            ...
            _ => {
                if let Err(_) = client_stream.send_error_message("Can't join, game has started".into(), MessageErrType::InvalidReq) {
    ...
    fn add_player_and_send_ack(&mut self, player_id: String, client_stream: StreamSender<ProtoPreGameMessage>) {
        ...
        self.players.send_pre_game_message(&player_id, ProtoJoinGameAck {
            game_type: ProtoGameType::LoveLetter.into(),
            host_player_id,
            other_player_ids,
        })
    }
